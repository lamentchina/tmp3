...
var tools = {
  ...
  lrcinit: function() {
    ...
    // var lrc = 'lrcs/141030.lrc';
    var lrc = 'lrcs/141105.lrc'
    // var lrc = 'lrcs/1.lrc'
    //ajax获取歌词lrc文件
    loadLrc(lrc);
  ...
  lrcm: function(atime, ctime) {
    ...
    var percent = tools.ctimer()/tools.atimer();
    var perdeg = percent * 360;
    if (percent < 0.5) {
      $process.css({
        'background-image':'linear-gradient(90deg, #5d6771 50%, transparent 50%, transparent), linear-gradient('+(90+perdeg)+'deg, #38B9F0 50%, #5d6771 50%, #5d6771)'
      })
    } else {
      $process.css({
        'background-image':'linear-gradient('+(-90+perdeg)+'deg, #38B9F0 50%, transparent 50%, transparent), linear-gradient(270deg, #38B9F0 50%, #5d6771 50%, #5d6771)'
      })
    }
